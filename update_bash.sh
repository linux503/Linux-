...
#Company:     AnchNet Computing
#Support the version: CentOS RedHat Ubuntu Debian     
#########################################################
...
n=`env x='() { :;}; echo vulnerable' bash -c "echo this is a test" |grep vulnerable |wc -l`
if [ $n -eq 1 ]; then
case "$os_release" in
redhat5|centos5|redhat6|centos6)
yum update -y bash &> /tmp/bash_install
;;
ubuntu10|ubuntu1204|ubuntu1210|debian6)
apt-get install bash &> /tmp/bash_install
;;
*)
echo "Unknown system"
;;
